...
var socketModule = {
    ...
    processGameRule: function (obj) {
        if (obj.data.ticket_type) {
            appData.ruleInfo.ticket = Math.floor(obj.data.ticket_type);
    ...
    processPrepareJoinRoom: function (obj) {
        if (obj.data.is_club) {
            if (obj.data.is_club == 1) {
                ...
                appData.applyInfo.club_creator = obj.data.room_creator;
                return;
            }
        }
    ...
    processUpdateAccountShow: function (obj) {
        for (var i = 0; i < appData.player.length; i++) {
            if (appData.player[i].account_id == obj.data.account_id) {
                console.log("processUpdateAccountShow:", obj.data.account_id);
                console.log("i:", i);
                console.log("appData.player.length:", appData.player.length);
    ...
    processBroadcastVoice: function (obj) {
        for (var i = 0; i < globalData.maxCount; i++) {
            if (appData.player[i].account_id == obj.data.account_id && i != 0) {
                if (appData.player[i].sex == 1) {
                    m4aAudioPlay("message" + obj.data.voice_num);
                } else {
                    m4aAudioPlay("message" + obj.data.voice_num + '_1');
                }
...
var viewMethods = {
    ...
    resetCardOver: function (num) {
        ...
        } else if (num == 11 || num == 12 || num == 13 || num == 14 || num == 15 || num == 16 || num == 17 || num == 18 || num == 19 || num == 20 || num == 21) {
    ...
    roundEnd: function () {
        //window.location.href = globalData.baseUrl + 'home/cn?i=' + globalData.roomNumber + '_&v=' + (new Date().getTime());
        //window.location.href = DynLoading
    ...
    updateAllPlayerStatus: function () {
        for (var i = 0; i < appData.player.length; i++) {
            ...
            if (appData.player[i].card_type >= 1) {
                ...
                appData.player[i].bullImg = globalData.fileUrl + "fiesc/images/bull13_yh/bull" + imgIndex + ".png";
            }
            if (appData.player[i].account_status == 4) {
    ...
    clearBanker: function () {
        ...
        if (appData.bankerArray.length < 6) {
            appData.bankerAnimateDuration = parseInt(1400 / totalCount); //3000
        } else {
            appData.bankerAnimateDuration = parseInt(2400 / totalCount); //5000
        }
    },
    ...
    showMemberScore: function (isShow) {
        if (isShow) {
            ...
            $(".memberScoreText14").show();
            $(".memberScoreText15").show();
            $(".memberScoreText16").show();
            $(".memberScoreText17").show();
        } else {
            ...
            $(".memberScoreText14").hide();
            $(".memberScoreText15").hide();
            $(".memberScoreText16").hide();
            $(".memberScoreText17").hide();
        }
    },
    winAnimate: function (obj) {
        ...
        for (var i = 0; i < loserNums.length; i++) {
            for (var j = 0; j < 5; j++) {
                $(".memberCoin" + loserNums[i] + j).animate(memCoin[appData.bankerPlayerNum], 240 + 240 * j);
            }
            setTimeout(function () {
        ...
        if (winnerNums.length >= 1) {
            setTimeout(function () {
                ...
                for (var i = 0; i < winnerNums.length; i++) {
                    for (var j = 0; j < 5; j++) {
                        $(".memberCoin" + winnerNums[i] + j).animate(memCoin[winnerNums[i]], 240 + 240 * j);
                    }
                }
                setTimeout(function () {
                    mp3AudioPlay("audioCoin", appData.player[0].sex);
    ...
    finishWinAnimate: function (obj) {
        ...
        $(".memberScoreText13").fadeIn(200);
        $(".memberScoreText14").fadeIn(200);
        $(".memberScoreText15").fadeIn(200);
        $(".memberScoreText16").fadeIn(200);
        $(".memberScoreText17").fadeIn(200, function () {
            ...
            setTimeout(function () {
                ...
                $(".memberScoreText1").fadeOut("fast");
                $(".memberScoreText2").fadeOut("fast");
                $(".memberScoreText3").fadeOut("fast");
                $(".memberScoreText4").fadeOut("fast");
                $(".memberScoreText5").fadeOut("fast");
                $(".memberScoreText6").fadeOut("fast");
                $(".memberScoreText7").fadeOut("fast");
                $(".memberScoreText8").fadeOut("fast");
                $(".memberScoreText9").fadeOut("fast");
                $(".memberScoreText10").fadeOut("fast");
                $(".memberScoreText11").fadeOut("fast");
                $(".memberScoreText12").fadeOut("fast");
                $(".memberScoreText13").fadeOut("fast");
                $(".memberScoreText14").fadeOut("fast");
                $(".memberScoreText15").fadeOut("fast");
                $(".memberScoreText16").fadeOut("fast");
                $(".memberScoreText17").fadeOut("fast");
