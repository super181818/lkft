...
var httpModule = {
    ...
    sendLog: function () {
        ...
        Vue.http.post(globalData.baseUrl + "api/log", postData).then(function (e) {
        }, function (e) {
        });
    }
};
var socketModule = {
    ...
    sendPrepareJoinRoom: function () {
        socketModule.sendData({
            operation: wsOperation.PrepareJoinRoom, account_id: userData.accountId, session: globalData.session,
    ...
    sendBroadcastVoice: function (e) {
        socketModule.sendData({
            ...
            data: {
                ...
                voice_num: e, token: globalData.tk
            }
        });
    },
    ...
    processPrepareJoinRoom: function (obj) {
        ...
        console.log("curr_circle", appData.curr_circle);
        ...
        // if(localStorage.isFirstRound==='true'){
        //     appData.isFirstRound=true;
        // }else{
        //     appData.isFirstRound=false;
        // }
    ...
    processRefreshRoom: function (e) {
        ...
        for (t = 0; t < 9; t++) appData.player.push({
            num: t + 1,
            serial_num: 0,
            account_id: 0,
            account_status: 0,
        ...
        for (t = 0; t < appData.player.length; t++) t <= appData.player.length - Math.ceil(e.data.serial_num) ?
            appData.player[t].serial_num = t + Math.ceil(e.data.serial_num) : appData.player[t].serial_num =
                t + Math.ceil(e.data.serial_num) - appData.player.length;
    ...
    processAllGamerInfo: function (obj) {
        ...
        for (var i = 0; i < 9; i++) {
            for (var j = 0; j < obj.data.length; j++) {
                ...
                if (appData.player[i].account_id == userData.accountId && appData.isAutoReady == 1 && appData.isWatching != 1 && appData.game.status == 1) {
                    methods.imReady();
                }
            }
        }
            ...
            for (var j = 0; j < obj.data.length; j++) {
                if (appData.player[i].account_id == obj.data[j].account_id) {
                    ...
                    if (appData.player[i].playing_status > 1) {
                        currentPlayerNum = i;
                        if (0 == i) {   //看牌条件
                            //appData.game.can_open = Math.ceil(obj.data[j].can_open);
    ...
    processNotyChooseChip: function (obj) {
        ...
        // if(userData.accountId==obj.data.account_id){
        //     httpModule.sendLog();
        // }
    ...
    processPKCard: function (e) {
        ...
        (appData.player[a].is_pk = !0, n = a);
        appData.game.score = appData.game.score + Math.ceil(e.data.score), viewMethods.playerPK(t, n)
    },
    ...
    processHuanPai: function (obj) {
        console.log("processHuanPai", obj);
        ...
        // appData.player[0].card[appData.changeCardNum]=obj.data.new_card;
        console.log("玩家的牌", appData.player[0].card);
        appData.player[0].card_type = obj.data.card_type;
        appData.showSwopBtn = false;
    },
    processhuanpaiNotify: function (obj) {
        console.log("processhuanpaiNotify", obj);
            ...
            for (var i = 0; i < 9; i++) {
                for (var j = 0; j < obj.data.length; j++) {
                    if (appData.player[i].account_id == obj.data[j].account_id) {
                        ...
                        // console.log("玩家",i,"的牌：",appData.player[i].card)
                    }
                }
            }
    ...
    processLastScoreboard: function (e) {
        if (void 0 != e) {
            console.log(e);
            try {
                var t = new Date(1e3 * parseInt(e.time)), n = t.getFullYear() + "-", a = t.getMonth() + 1 + "-",
                    i = t.getDate() + " ", o = t.getHours(), r = t.getMinutes(),
                    c = ":";
                r < 10 && (c += 0);
                var l = n + a + i + o + c + r;
                appData.playerBoard.round = e.game_num, appData.playerBoard.record = l,
                    appData.playerBoard.score = [], void 0 != e.total_num && null != e.total_num && "" != e.total_num && (appData.game.total_num = e.total_num);
                var u = e.scoreboard;
                for (s in u) {
                    var p = 0;
                    userData.accountId == u[s].account_id && (p = 1),
                        appData.playerBoard.score.push({
                            account_id: u[s].account_id, nickname: u[s].name, "avatar": u[s].avatar,
                            account_score: Math.ceil(u[s].score), num: p
                        })
                }
                chooseBigWinner(),
                    $(".ranking .rankBack").css("opacity", "1"), $(".roundEndShow").show(), $(".ranking").show(),
                    canvas(), $("#endCreateRoomBtn").show()
            } catch (e) {
                console.log(e)
            }
        }
...
var viewMethods = {
    ...
    clickShowAlert: function (e, t) {
        console.log(e, t),
            ...
            $(".alert .mainPart").css("height", a + "px"), $(".alert .mainPart").css("margin-top", "-" + a / 2 + "px"), $(".alert .mainPart .backImg .blackImg").css("height", i + "px"), $(".alert .mainPart .alertText").css("top", o + "px")
        }, 0)
    },
    clickCloseAlert: function () {
        22 == appData.alertType ? (appData.isShowGameAlert = !1, httpModule.getInfo()) : appData.isShowGameAlert = !1;
    ...
    cardOver2: function (num) {
        if (appData.ruleInfo.play_mode == 2) { //明牌模式
            $(".cardDeal .mcard0").addClass("card-flipped");
    ...
    gameOver: function (e, t, n, a) {
        ...
        setTimeout(function () {
            for (var e = 0, n = 0; n < appData.player.length; n++) appData.player[n].is_win &&
            (appData.player[n].win_show = !0, 0 == n && mp3AudioPlay("win"), 0 == e && (e = 1, numD = appData.player[n].num,
                setTimeout(function () {
                    viewMethods.selectCoin(numD, t)
                }, 1500)));
            var i = new Date, o = "";
            o += i.getFullYear() + "-", o += i.getMonth() + 1 + "-", o += i.getDate() + "  ", o += i.getHours() + ":",
    ...
    messageSay: function (e, t) {
        appData.player[e].messageOn = !0, appData.player[e].messageText = appData.message[t].text,
            setTimeout(function () {
                appData.player[e].messageOn = !1
            }, 2500)
    },
    closeEnd: function () {
        $(".ranking .rankBack").css("opacity", "0.7"), $(".end").hide(),
            $(".roundEndShow").hide(), $(".ranking").hide(), window.location.reload()
    },
    roundEnd2222: function () {
        for (var e = 0; e < appData.player.length; e++) 0 == appData.player[e].online_status && (appData.player[e].account_id = 0, appData.player[e].playing_status = 0,
        ...
        chooseBigWinner(), $(".ranking .rankBack").css("opacity", "1"), $(".roundEndShow").show(),
            setTimeout(function () {
                $(".ranking").show(), canvas()
            }, 3500)
    },
    roundEnd: function () {
        setTimeout(function () {
            DynLoading.goto('flower9', 'i=' + globalData.roomNumber); 
            //window.location.href = globalData.baseUrl + 'home/zh?i=' + globalData.roomNumber + '_&v=' + (new Date().getTime());
        }, 3500)
    },
    timeCountDown: function () {
        if (1 != isTimeLimitShow)
            return appData.game.time <= 0 ? (isTimeLimitShow = !1, 0) : (isTimeLimitShow = !0, appData.game.time--,
                void (timeLimit = setTimeout(function () {
    ...
    choose: function (type, num) {
        ...
        if (type == 1) {
            ...
            // appData.isFirstRound=false;
            // if(appData.play_type==4){
            //     localStorage.isFirstRound=appData.isFirstRound
            // }
            if (appData.play_type == 4) {
                appData.swopBtnActive = false;
                appData.canChooseCard = false;
...
var connectSocket = function (e, t, n, a, i) {
    (ws = new WebSocket(e)).onopen = t, ws.onmessage = n, ws.onclose = a, ws.onerror = i
};
...
var methods = {
    ...
    swopCard: function () {
        ...
        // if(appData.ruleInfo.disable_pk==1&&appData.ruleInfo.disable_look==1){
        //     appData.isFirstRound=true;
        //     localStorage.isFirstRound=appData.isFirstRound;
        // }
        console.log("点击换牌按钮");
...
var wxModule = {
    config: function () {
        ...
        //     title:"九人炸金花(房间号:"+globalData.roomNumber+")",
        //     desc:shareContent,
