...
app.controller("myCtrl", function ($scope, $http) {
    ...
    $scope.connectSocket = function (socket, type) {
        ...
        ws.onopen = function () {
            ...
            if (type == 1) {
                ...
                $scope.func = 'cn';
                $scope.max_count_title = $scope.maxCount[$scope.createInfo.bbullx.max_count_type - 1] + '人牛牛';
            } else if (type == 2) {
                if (globalData.p_type == 2) {
                    if ($scope.createInfo.bbullx.is_custom_score == 1) {
                ...
                $scope.func = 'n';
                $scope.max_count_title = $scope.maxCount[$scope.createInfo.bullx.max_count_type - 1] + '人牛牛';
            } else if (type == 3) {
                sendData = {
                    "operation": "CreateRoom",
                    "data": {
                ...
                $scope.func = 'zh';
                $scope.max_count_title = $scope.maxCount[$scope.createInfo.flower.max_count_type - 1] + '人炸金花';
            } else if (type == 5) {
                sendData = {
                    "operation": "CreateRoom",
                    "data": {
    ...
    $scope.createInfo = {
        ...
        "dxbull": localStorage.dxbull ? JSON.parse(localStorage.dxbull) : {
            ...
            "bet_type": 1,
            "max_count_type": 2,
            "odds_num": 2,   //规则 1: 牛牛x3牛九x2牛八x2      2: 牛牛x4牛九x3牛八x2牛七x2
            "is_laizi": 0,      //是否有赖子
            "is_cardfive": 1, //牌型 五花牛(5倍)  1表示默认勾选
            "is_cardbomb": 1, //牌型 炸弹牛(6倍)
            "is_cardtiny": 1, //牌型 五小牛(8倍)
